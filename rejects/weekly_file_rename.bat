******************************************************************{
  "error": {
    "message": "This model's maximum context length is 4097 tokens, however you requested 5200 tokens (5200 in your prompt; 0 for the completion). Please reduce your prompt; or completion length.",
...
echo on
rem ################################################################################################################################################
rem FileName:          weekly_file_rename.bat
rem Description:       
rem Author:            
rem Date Created:       
...
D:\java8\bin\jar xf JDA_SALESORDER_%run_date%*.zip >> %batch_log%\%log_file% 2>>&1
cd /d D:\JDABatch\batch\cmd
...
COPY /b %sftp_inbox%\JDA_SALESORDER_%run_date%*.dat %batch_inbox%\jbi003_load_salesorder.dat >> %batch_log%\%log_file% 2>>&1
COPY /b %sftp_inbox%\JDA_SALESORDERMAPPING_%run_date%*.dat %batch_inbox%\jbi004_load_salesordermap.dat >> %batch_log%\%log_file% 2>>&1
...
del %sftp_inbox%\JDA_SALESORDER_%run_date%*.dat >> %batch_log%\%log_file%
...
MOVE /Y %sftp_inbox%\JDA_SALESORDER_%run_date%*.zip %batch_inbox_archive% >> %batch_log%\%log_file% 2>>&1
MOVE /Y %sftp_inbox%\JDA_SALESORDERMAPPING_%run_date%*.dat %batch_inbox_archive% >> %batch_log%\%log_file% 2>>&1
...
echo WEEKLY_FILE_RENAME COMPLETED: %date% %time% >> %batch_log%\%log_file% 2>>&1
ECHO ************************** WEEKLY_FILE_RENAME ENDED ******************************  >> %batch_log%\%log_file% 2>>&1
echo WEEKLY_FILE_RENAME ENDED: %date% %time% >> %batch_log%\%log_file% 2>>&1
Echo *************************************************************************************   >> %batch_log%\%log_file% 2>>&1
...
IF NOT EXIST %sftp_inbox%\JDA_SALESORDER_%run_date%*.zip GOTO end
IF NOT EXIST %sftp_inbox%\JDA_SALESORDERMAPPING_%run_date%*.dat GOTO end
...
:end 
echo *** WEEKLY_FILE_RENAME DAT FILES ARE MISSING: %date% %time% >> %batch_log%\%log_file% 2>>&1
