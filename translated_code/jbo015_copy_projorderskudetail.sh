...
datafile="JDA_PROJORDERSKUDETAIL_"$run_time

echo ---------------------------------------------- >> %LOG_FILE% 2>>&1
echo EXTRACT PROCESS Started AT $(date +"%T"), $(date +"%D") >> %LOG_FILE% 2>>&1
echo Output will be written TO FILE $datafile.dat>> %LOG_FILE% 2>>&1
echo. >> %LOG_FILE% 2>>&1
sqlplus -s $BATCH_LOGIN @$SQL_DIR/jbo015_extract_projorderskudetail.sql $datafile >> %LOG_FILE% 2>>&1
...
/usr/java/jdk1.8.0_181/bin/jar -c