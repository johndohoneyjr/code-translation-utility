...
datafile="JDA_MEANVALUEADJ_EXTRACT_$run_time"

echo "----------------------------------------------" >> "$LOG_FILE" 2>&1
echo "EXTRACT PROCESS Started AT $(date +"%T"), $(date +"%D")" >> "$LOG_FILE" 2>&1
echo "Output will be written TO FILE $datafile.dat" >> "$LOG_FILE" 2>&1
echo "" >> "$LOG_FILE" 2>&1
sqlplus -s "$BATCH_LOGIN" "@$SQL_DIR/jbo029_extract_meanvalueadj.sql" "$datafile" >> "$LOG_FILE" 2>&1
...
cd "$SFTP_OUT_DIR" || exit

/java8/bin/jar -cMf "$datafile.