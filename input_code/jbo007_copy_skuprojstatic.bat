ECHO ON
REM ####################################################################
REM FileName:          jbo007_copy_skuprojstatic.bat
REM Description:       Prepare the skuprojstatic Export for HDS
REM Note:
REM ####################################################################
...
SET datafile=JDA_SKUPROJSTATIC_%run_time%
...
ECHO. >> %LOG_FILE% 2>>&1
CALL sqlplus -s %BATCH_LOGIN% @%SQL_DIR%\jbo007_extract_skuprojstatic.sql %datafile% >> %LOG_FILE% 2>>&1
