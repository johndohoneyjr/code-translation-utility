ECHO ON
REM ####################################################################
REM FileName:          jbo025_copy_da_skuprojstatic.bat
REM Description:       Prepare the DA SKUPROJSTATIC Export for HDS
REM Note:
REM ####################################################################
...
REM --------------------------------------------------------------------
rem  07-Mar-2019    REDACTED          Initial Version
REM
REM ####################################################################
...
REM SET run_time=%date:~4,2%%date:~7,2%%date:~10,4%_%time:~0,2%%time:~3,2%%time:~6,2%
...
SET datafile=JDA_DASKUPROJSTATIC_%run_time%
...
ECHO. >> %LOG_FILE% 2>>&1
CALL sqlplus -s %BATCH_LOGIN% @%SQL_DIR%\jbo025_extract_da_skuprojstatic.sql %datafile% >> %LOG_FILE% 2>>&1
