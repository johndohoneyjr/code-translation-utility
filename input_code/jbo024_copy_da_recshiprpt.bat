ECHO ON
REM ####################################################################
REM FileName:          jbo024_copy_da_recshiprpt.bat
REM Description:       Prepare the DA Recship Report Export for HDS
REM Note:
REM ####################################################################
...
SET datafile=JDA_DARECSHIPRPT_%run_time%
...
ECHO. >> %LOG_FILE% 2>>&1
CALL sqlplus -s %BATCH_LOGIN% @%SQL_DIR%\jbo024_extract_da_recshiprpt.sql %datafile% >> %LOG_FILE% 2>>&1
