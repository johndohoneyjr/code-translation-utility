ECHO ON
REM ####################################################################
REM FileName:          jbo005_copy_hist.bat
REM Description:       Prepare the Hist Export for HDS
REM Note:
REM ####################################################################
...
SET datafile=JDA_HIST_%run_time%
...
ECHO. >> %LOG_FILE% 2>>&1
CALL sqlplus -s %BATCH_LOGIN% @%SQL_DIR%\jbo005_extract_hist.sql %datafile% >> %LOG_FILE% 2>>&1
