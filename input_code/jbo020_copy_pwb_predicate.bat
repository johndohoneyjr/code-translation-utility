ECHO ON
REM ####################################################################
REM FileName:          jbo020_copy_pwb_predicate.bat
REM Description:       Prepare the PWB Prediccate extract
REM Note:
REM ####################################################################
...
SET datafile=JDA_PWB_PREDICATE_%run_time%
...
ECHO. >> %LOG_FILE% 2>>&1
CALL sqlplus -s %BATCH_LOGIN% @%SQL_DIR%\jbo020_extract_pwb_predicate.sql %datafile% >> %LOG_FILE% 2>>&1
