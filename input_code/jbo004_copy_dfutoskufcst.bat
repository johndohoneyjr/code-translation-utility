ECHO ON
REM ####################################################################
REM FileName:          jbo004_copy_dfutoskufcst.bat
REM Description:       Prepare the DfuttoSku Fcst Export for HDS
REM Note:
REM ####################################################################
...
ECHO **************************DFUTOSKUFCST EXTRACT STARTED************************************************  >> %LOG_FILE% 2>>&1
...
SET datafile=JDA_DFUTOSKUFCST_%run_time%
...
rem if %DAY%==Tue ( 
if %DAY%==Tue (  
CALL sqlplus -s %BATCH_LOGIN% @%SQL_DIR%\jbo004_extract_dfutoskufcst.sql %datafile% >> %LOG_FILE% 2>>&1
echo Weekly DFUTOSKUFCST Extract Completed >> %LOG_FILE% 2>>&1
 ) else ( 
 echo Extract is scheduled to run as part of Monday batch only >> %LOG_FILE% 2>>&1
 GOTO end
 )
...
rem COPY /b %SFTP_OUT_DIR%\%datafile%.dat %OUT_ARCH_DIR%\%datafile%.dat >> %LOG_FILE% 2>>&1
