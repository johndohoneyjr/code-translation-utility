ECHO ON
REM ####################################################################
REM FileName:          jbo001_copy_dmdunit.bat
REM Description:       Prepare the DmdUnit Export for HD Supply
REM Note:
REM ####################################################################
...
SET datafile=JDA_DMDUNIT_EXTRACT_%run_time%
...
ECHO. >> %LOG_FILE% 2>>&1
CALL sqlplus -s %BATCH_LOGIN% @%SQL_DIR%\jbo001_extract_dmdunit.sql %datafile% >> %LOG_FILE% 2>>&1
