ECHO ON
REM ####################################################################
REM FileName:          jbo011_copy_projordersku.bat
...
SET datafile=JDA_PROJORDERSKU_%run_time%
...
ECHO. >> %LOG_FILE% 2>>&1
CALL sqlplus -s %BATCH_LOGIN% @%SQL_DIR%\jbo011_projordersku_extract.sql %datafile% >> %LOG_FILE% 2>>&1
