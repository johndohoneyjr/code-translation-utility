ECHO ON
REM ####################################################################
REM FileName:          jbo019_copy_pwb_rule.bat
REM Description:       Prepare the PWB Rule extract
REM Note:
REM ####################################################################
...
REM --------------------------------------------------------------------
rem  24-Jun-2018    REDACTED            Initial Version
REM
REM ####################################################################



ECHO **************************PWB RULE EXTRACT STARTED************************************************  >> %LOG_FILE% 2>>&1
...
SET datafile=JDA_PWB_RULE_%run_time%
...
ECHO. >> %LOG_FILE% 2>>&1
CALL sqlplus -s %BATCH_LOGIN% @%SQL_DIR%\jbo019_extract_pwb_rule.sql %datafile% >> %LOG_FILE% 2>>&1
