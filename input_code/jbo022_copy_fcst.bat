ECHO ON
REM ####################################################################
REM FileName:          jbo022_copy_fcst.bat
REM Description:       Prepare the Fcst Export for HDS
REM Note:
REM ####################################################################
...
rem if %DAY%==%DAY% ( 
if %DAY%==Sun ( 
CALL sqlplus -s %BATCH_LOGIN% @%SQL_DIR%\jbo022_extract_fcst.sql %datafile% >> %LOG_FILE% 2>>&1
echo Weekly FCST Extract Completed >> %LOG_FILE% 2>>&1
) else ( 
echo Extract is scheduled to run on Saturdays batch only >> %LOG_FILE% 2>>&1
GOTO end
)
...
ECHO.  >> %LOG_FILE% 2>>&1
ECHO Extract File doesnt exist, please investigate... >> %LOG_FILE% 2>>&1
GOTO errorblock
)
