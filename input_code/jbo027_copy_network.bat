ECHO ON
REM ####################################################################
REM FileName:          jbo027_copy_network.bat
...
REM --------------------------------------------------------------------
rem  29-Nov-2019    REDACTED        Initial Version
REM
REM ####################################################################
...
SET datafile=JDA_NETWORK_EXTRACT_%run_time%
...
ECHO. >> %LOG_FILE% 2>>&1
CALL sqlplus -s %BATCH_LOGIN% @%SQL_DIR%\jbo027_extract_network.sql %datafile% >> %LOG_FILE% 2>>&1
