ECHO ON
REM ####################################################################
REM FileName:          jbo005_copy_hist_fullload.bat
REM Description:       Prepare the History Export for HDS
REM Note:
REM ####################################################################
...
SET datafile=JDA_HISTFULLLOAD_%run_time%
...
rem if %DAY%==Thu (
if %DAY%==Wed (  
CALL sqlplus -s %BATCH_LOGIN% @%SQL_DIR%\jbo005_extract_hist_fullload.sql %datafile% >> %LOG_FILE% 2>>&1
echo HIST Extract for 3 years Completed >> %LOG_FILE% 2>>&1
 ) else ( 
 echo Extract is scheduled to run as part of Tuesdays batch only >> %LOG_FILE% 2>>&1
 GOTO end
 )
