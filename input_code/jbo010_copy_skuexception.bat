ECHO ON
REM ####################################################################
REM FileName:          jbo010_copy_skuexception.bat
...
SET datafile=JDA_SKUEXCEPTION_%run_time%
...
ECHO. >> %LOG_FILE% 2>>&1
CALL sqlplus -s %BATCH_LOGIN% @%SQL_DIR%\jbo010_skuexception_extract.sql %datafile% >> %LOG_FILE% 2>>&1
