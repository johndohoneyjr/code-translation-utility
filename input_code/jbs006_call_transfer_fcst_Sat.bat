ECHO ON
REM ####################################################################
REM FileName:          jbs006_call_transfer_fcst.bat
REM Description:       Call Transfer Forecast Processes
REM Note:
REM ####################################################################
...
if %DAY%==Sun ( 
SET job_name=%XML_DIR%\jbs006_transfer_fcst_weekly
call %FWK_DIR%\run_process.bat jbs006_transfer_fcst_weekly
echo Weekly Transfer Forecast process Completed >> %LOG_FILE% 2>>&1
) else ( 
SET job_name=%XML_DIR%\jbs006_transfer_fcst_daily
call %FWK_DIR%\run_process.bat jbs006_transfer_fcst_daily
