ECHO ON
REM ####################################################################
REM FileName:          jbo008_copy_pi_fcst.bat
...
SET datafile=JDA_PI_FCST_EXTRACT_%run_time%
...
ECHO. >> %LOG_FILE% 2>>&1
CALL sqlplus -s %BATCH_LOGIN% @%SQL_DIR%\jbo008_pi_fcst_extract.sql %datafile% >> %LOG_FILE% 2>>&1
