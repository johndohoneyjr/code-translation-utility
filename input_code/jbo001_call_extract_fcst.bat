echo on
rem ####################################################################
rem FileName:          jbo001_call_extract_fcst.bat
...
rem --------------------------------------------------------------------
rem  16-MAY-2017	REDACTED			Initial Version
rem 
rem ####################################################################
...
set datafile=JDA_FCST_%run_time%
set logfile=%batch_home%\log\jbo001_call_extract_fcst_%TIMESTAMP%.log
...
rem call %batch_home%\fwk\run_process.bat jbo001_extract_fcst %datafile% >> %logfile% 2>>&1
call sqlplus -s %BATCHUSER%/%BATCHPWD%@%ORACLE_SID% @%sql_home%\jbo001_extract_fcst.sql %datafile% >> %logfile% 2>>&1
